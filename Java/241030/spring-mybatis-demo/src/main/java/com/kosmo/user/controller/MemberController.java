...
//@Autowired,@Inject, @Resource ==> DI할때 사용하는 어노테이션
...
public class MemberController {
    ...
    public String joinProcess(MemberDTO user, Model model){
        //html의 input name과 MemberDTO객체의 property명을 같게 준다면
        //스프링이 알아서 MemberDTO객체에 사용자가 입력한 값을 넣어준다.
        log.info("user:{}",user);
        //유효성 체크
        if(user.getName()==null||user.getUserId()==null||user.getUserPw()==null
                ||user.getName().trim().isBlank()||user.getUserId().trim().isBlank()||user.getUserPw().trim().isBlank()){
            return "redirect:signup";
            //redirect:을 뷰네임에 접두어로 붙어주면 redirect방식으로 이동
            //디폴트 이동방식은 forward이다
        }
    ...
    public String memberUpdate(MemberDTO user,Model model){
        log.info("user={}",user);
        if(user.getIdx()==0||user.getName().trim().isBlank()||user.getUserId().trim().isBlank()){
            return "redirect:users";
        }
    ...
    public String idCheckResult(Model model,@RequestParam(defaultValue = "") String userId){
        ...
        String msg = (isUse)? userId+"는 사용 가능합니다":userId + "는 이미 사용중인 아이디 입니다.";
