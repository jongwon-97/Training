...
//			return;
		...
		//페이지 이동
		//[1] redirect방식으로 이동하는 방법<== 20% : res.sendRedirect("뷰페이지");
		//[2] forward방식으로 이동하는 방법 <== 80% : RequestDispacher의 forward()메서드를 이용
		//forward방식 이동
		RequestDispatcher disp=req.getRequestDispatcher(viewPage);
		disp.forward(req, res);//forward방식으로 이동 (서버 내부에서 이동). req, res를 가져가서
		//viewPage(jsp)와 공유한다
